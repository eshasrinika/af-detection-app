...
use_sample = st.button("Use Sample ECG")
...
if uploaded_file is not None or use_sample:
    ...
    if uploaded_file is not None:
        ...
        sample_choice = random.choice(["af", "nsr"])

        if sample_choice == "af":
            if os.path.exists("af_sample.npy"):
                ecg = np.load("af_sample.npy")
                st.info("Using AF sample ECG")
            else:
                st.error("af_sample.npy not found")
                st.stop()

        else:
            if os.path.exists("nsr_sample.npy"):
                ecg = np.load("nsr_sample.npy")
                st.info("Using NSR sample ECG")
            else:
                st.error("nsr_sample.npy not found")
                st.stop()
